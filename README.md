...
- **Get Started popup form** — when Elementor popup `#7822` opens, populates its `qualification_of_interest` and `qualification_category` selects from the `qualifications` posts and the `qualification-category` taxonomy, pre-selects them from the current qualification page (title, category, and ACF `qualification_type` / `qualification_level`), upgrades them to searchable Select2, and filters the qualification list by the chosen category (client-side).
...
- The Get Started popup form targets Elementor popup `#7822` and its `form-field-qualification_of_interest`, `form-field-qualification_category`, `form-field-qualification_type` and `form-field-qualification_level` fields (Elementor prefixes form field ids with `form-field-`). It reads the `qualifications` post type and the **`qualification-category`** taxonomy (note: hyphenated — distinct from WPSL's `wpsl_store_category`), and the per-post `qualification_type` / `qualification_level` come from ACF (the calls are wrapped in `function_exists('get_field')`, so an inactive ACF degrades gracefully rather than fatals). The searchable dropdowns require **Select2** to be enqueued by the theme/Elementor. Adjust the popup id, field ids, post type or taxonomy in the source if your site differs.
